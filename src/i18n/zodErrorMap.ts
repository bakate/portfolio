...
function getErrorMessage(lang: LanguageCode, key: keyof typeof ui[LanguageCode]['zodErrors'], params?: Record<string, string | number>): string {
  let message: string = ui[lang]?.zodErrors?.[key] || ui[defaultLanguage]?.zodErrors?.[key] || ''; // Initialize with empty string if key not found

  if (!message) { // Check if message is still empty after primary lookups
...
const customErrorMap = (lang: LanguageCode): z.ZodErrorMap => (issue, ctx) => {
  let message: string;
  ...
  switch (issue.code) {
    case z.ZodIssueCode.invalid_type:
      if (issue.received === ZodParsedType.undefined) {
        ...
        message = getErrorMessage(lang, 'invalid_type', { received: String(issue.received) });
      }
      break;
    ...
    case z.ZodIssueCode.too_small:
      if (issue.type === 'string') {
        message = getErrorMessage(lang, 'too_small_string_minimum', { minimum: String(issue.minimum) });
      } else {
    ...
    case z.ZodIssueCode.too_big:
      if (issue.type === 'string') {
        message = getErrorMessage(lang, 'too_big_string_maximum', { maximum: String(issue.maximum) });
      } else {
